  ...
  return (
    <div className="animate-in" style={{ paddingBottom: 64 }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800 }}>🎙️ Narração e Marketing</h1>
        <p style={{ color: "var(--text-muted)", fontSize: 14, marginTop: 4 }}>
          Selecione o funil desejado para personalizar os roteiros dinâmicos (TTS) de cada nível.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 24 }}>
          {funnels.map(funnel => (
            <Link key={funnel.id} href={`/scripts/${funnel.id}`} style={{ textDecoration: "none" }}>
              <div 
                className="card" 
                style={{ 
                  padding: 24, 
